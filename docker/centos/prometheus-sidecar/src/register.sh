...
grafana_status(){
    response=$(curl -sI \
                    "admin:${G_ADMIN_PASS}@${G_HOST}:${G_PORT}/api/datasources" \
    ...
    case $response in
        200)
            ...
            ;;
        *)
            echo "{'message': '${response} error: unable to connect with Grafana at ${G_HOST}:${G_PORT}'}"
            return 1
            ;;
    esac
}
...
echo "{'message': 'waiting for Grafana at ${G_HOST}:${G_PORT} to be ready'}"
for i in {0..$G_WAIT}; do
    sleep 10
    if grafana_status; then
        echo "{'message': 'success: connected with Grafana at ${G_HOST}:${G_PORT}'}"
        break
    elif (( i == $G_WAIT )); then
        echo "{'message': 'failed: Grafana at ${G_HOST}:${G_PORT} cannot be connected to'}"
        exit 1
    fi
done
