...
public class ConfigurationFileManager {
  ...
  protected void loadLocation(File node) {
    try (Scanner scanner = new Scanner(node)) {
      ...
      while (scanner.hasNextLine()) {
        ...
        int calCalc = Integer.parseInt(lineWords.get(1));
        ...
        if (location.isEmpty()) {
          ...
          StringTokenizer st = new StringTokenizer(lineWords.get(6), "_,");
  ...
  private boolean doFileReplacement(Replacement replacement) {
    ...
    File manipulateThis = new File(replacement.getFileName());
    File original = new File(this.getClass().getClassLoader().getResource(replacement.getOriginalEntry()).getPath());
    File replacementFile = new File(this.getClass().getClassLoader().getResource(replacement.getReplacementEntry()).getPath());
    ...
    try (Scanner scanner = new Scanner(manipulateThis); Scanner scanner1 = new Scanner(original);
         Scanner scanner2 = new Scanner(replacementFile)) {
      while (scanner.hasNextLine()) {
        maniFile.add(scanner.nextLine());
      }
      while (scanner1.hasNextLine()) {
        originalFile.add(scanner1.nextLine());
      }
      while (scanner2.hasNextLine()) {
        replaceFile.add(scanner2.nextLine());
      }
      //Compare...
      if (maniFile.containsAll(originalFile)) {
        //we have a match!
        //where is the match?
        int indexStart = maniFile.indexOf(originalFile.get(0));
        int indexEnd = maniFile.indexOf(originalFile.get(originalFile.size()-1));

        //small sanity check to make sure indexes line up and there is nothing in-between...
        if ((indexEnd - indexStart) == originalFile.size()-1) {
          //this should be good...
          maniFile.removeAll(originalFile);
  ...
  private boolean doAddition(Replacement replacement, String station) {
    boolean result = false;
    File manipulateThis = new File(String.format("%s/%s/%s", scanDir.getPath(),
        station, replacement.getFileName()));
    String original = replacement.getOriginalEntry();
    String replace = replacement.getReplacementEntry();

    try (Scanner scanner = new Scanner(manipulateThis)) {
      ArrayList<String> currentFile = new ArrayList<>();
      String line = "";
      while (scanner.hasNextLine()) {
        line = scanner.nextLine();
        if (line.contains(original)) {
          ...
          String line2 = line.replace(original, replace);
  ...
  protected void writeNetworkDatFile () {
    ...
    String line = String.format("ww   %-22s -1 %s %s",
    ...
    File node = new File("network.dat");
    try (FileWriter tmp = new FileWriter(scanDir.getPath() + "/" + node.getName(), false)) {
      ...
      logger.error("writeNetworkDatFile: IOException occurred. File: {}", node.getName());
    }
  }
