...
import gms.dataacquisition.cd11.rsdf.processor.Cd11AcquiredChannelEnvironmentIssuesParser;
...
public class Cd11DataFrameSoh {
  ...
  Cd11Frame transcribeSohToBytes() {
    ...
    for (int i = 0; i < dataPayload.getChannelSubframes().size(); i++) {
      byte[] channelStatusData = acquiredSohListToBytes(this.cd11AcquiredSohList.get(i), dataPayload
          .getChannelSubframes().get(i).channelStatusData);
      Cd11ChannelSubframe newChannelSubframe = new Cd11ChannelSubframe(
          dataPayload.getChannelSubframes().get(i), channelStatusData.length, channelStatusData);
  ...
  byte[] acquiredSohListToBytes(List<AcquiredChannelEnvironmentIssue<?>> sohList, byte[] oldBytes) {
    ...
    if (!sohList.isEmpty()) {
      ...
      for (byte b : byteIntArr) {
        newSohBytes[byteIdx] = b;
        byteIdx++;
      }
    }
