...
public abstract class Cd11Frame {
   ...
   */
  public byte[] toBytes() {
    byte[] headerBytes = getHeader().toBytes();
    byte[] frameBodyBytes = getPayload().toBytes();
    byte[] trailerBytes = getTrailer().toBytes();

    ByteBuffer frameBytes = ByteBuffer.allocate(
        headerBytes.length + frameBodyBytes.length + trailerBytes.length);
    frameBytes.put(headerBytes);
    frameBytes.put(frameBodyBytes);
    frameBytes.put(trailerBytes);
