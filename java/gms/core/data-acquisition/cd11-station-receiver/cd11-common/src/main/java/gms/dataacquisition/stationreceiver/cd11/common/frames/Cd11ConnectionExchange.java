...
public abstract class Cd11ConnectionExchange implements Cd11Payload {
  ...
  public byte[] toBytes() {
    ...
    output.putShort(getMajorVersion());
    output.putShort(getMinorVersion());
    output.put(padToLength(getStationOrResponderName(), 8).getBytes());
    output.put(padToLength(getStationOrResponderType(), 4).getBytes());
    output.put(padToLength(getServiceType(), 4).getBytes());
    output.putInt(getIpAddress());
    output.putChar((char) getPort()); // Convert Java "int" to "unsigned short".
    output.putInt(getSecondIpAddress()
        // To adhere to CD1.1 Spec, this field must be populated when serialized
        .orElse(0));
    output.putChar((char) getSecondPort() // Convert Java "int" to "unsigned short".
        // To adhere to CD1.1 Spec, this field must be populated when serialized
        .orElse(0));
  ...
  public static Builder withConfig(Cd11ConnectionConfig config){
    return builder()
      ...
      .setStationOrResponderName(config.getStationOrResponderName())
      .setStationOrResponderType(config.getStationOrResponderType())
