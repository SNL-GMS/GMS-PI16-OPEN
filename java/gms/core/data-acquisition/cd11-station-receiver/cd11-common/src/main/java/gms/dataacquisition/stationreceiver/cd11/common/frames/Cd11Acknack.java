...
public abstract class Cd11Acknack implements Cd11Payload {
  ...
  @Memoized
  public byte[] toBytes() {
    ByteBuffer output = ByteBuffer.allocate(Cd11Acknack.MINIMUM_FRAME_LENGTH +
      (getGapRanges().length * Long.BYTES));
    output.put(padToLength(getFrameSetAcked(), 20).getBytes());
    output.putLong(getLowestSeqNum());
    output.putLong(getHighestSeqNum());
    output.putInt(getGapCount());
