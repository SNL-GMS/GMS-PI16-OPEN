...
public abstract class Cd11OptionExchange implements Cd11Payload {
  ...
  @Memoized
  public byte[] toBytes() {
    int optionValuePaddedSize = calculatePaddedLength(getOptionValue().length(), 4);
    ByteBuffer output = ByteBuffer.allocate(
      Integer.BYTES +       // Option count.
        (2 * Integer.BYTES) + // Option type + Option size.
        optionValuePaddedSize // Option response
    );

    output.putInt(1); // Hard coded, since we only accept one option at this time.
    output.putInt(getOptionType());
    output.putInt(getOptionValue().length()); // Unpadded length of the option request.
    output.put(padToLength(getOptionValue(), optionValuePaddedSize).getBytes());
