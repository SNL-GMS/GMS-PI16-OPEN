...
public abstract class Cd11Trailer {
  ...
  public static Cd11Trailer read(ByteBuffer buffer) {
    int inAuthenticationKeyIdentifier = buffer.getInt();
    int inAuthenticationSize = buffer.getInt();
    int paddedAuthValSize = FrameUtilities
        .calculatePaddedLength(inAuthenticationSize, Integer.BYTES);
    byte[] inAuthenticationValue = FrameUtilities.readBytes(buffer, paddedAuthValSize);
    long inCommVerification = buffer.getLong();

    return Cd11Trailer.builder()
        .setAuthenticationKeyIdentifier(inAuthenticationKeyIdentifier)
        .setAuthenticationSize(inAuthenticationSize)
        .setAuthenticationValue(inAuthenticationValue)
        .setCommVerification(inCommVerification)
        .build();
  }
  ...
  public static Cd11Trailer fromBytes(int authenticationKeyIdentifier,
    ...
    int authenticationSize = 0; // TODO: Generate this value.
    byte[] authenticationValue = new byte[0]; // TODO: Generate this value.

    byte[] entireFrame = Bytes.concat(
        frameHeaderAndBody,
        Ints.toByteArray(authenticationKeyIdentifier),
        Ints.toByteArray(authenticationSize),
        authenticationValue,
        Longs
   ...
   */
  public byte[] toBytes() {
    int trailerAllocation = MINIMUM_TRAILER_LENGTH + getAuthenticationSize() + authValuePadding;
    ByteBuffer output = ByteBuffer
        .allocate(trailerAllocation);
    output.putInt(getAuthenticationKeyIdentifier());
    output.putInt(getAuthenticationSize());
    output.put(getAuthenticationValue());
    output.putLong(getCommVerification());
    return output.array();
  }
}
