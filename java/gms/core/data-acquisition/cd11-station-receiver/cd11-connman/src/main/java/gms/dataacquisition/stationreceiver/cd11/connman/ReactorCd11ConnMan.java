...
public class ReactorCd11ConnMan implements Cd11ConnMan {
  ...
  private static final StructuredLoggingWrapper logger = StructuredLoggingWrapper
      .create(LoggerFactory.getLogger(ReactorCd11ConnMan.class));
  ...
  private static final int FAILURE_RETRIES = 4;
  // Connman and System config
  private final Cd11ConnManConfig cd11ConnManProcessingConfiguration;
  private final SystemConfig cd11ConnManSystemConfiguration;
  private final int connectionManagerWellKnownPort;
  ...
  public void start() {
    ...
    Mono<?> monoTcpServer = TcpServer.create()
        ...
        .wiretap(logger.getWrappedLogger().isDebugEnabled())
        ...
        .doOnConnection(connection -> {
          ...
          connection.onDispose(() ->
              logger.debug("Connection Manager closed on well-known port {}",
                  connectionManagerWellKnownPort));
        })
        .bind();

    // Retry on failure or error on initial connection
    monoTcpServer.retry(FAILURE_RETRIES)
        .doOnError(e -> logger.error("Netty retries failed:", e))
        .doOnSuccess(s -> logger
            .info("Successfully connected on well-known port {}", connectionManagerWellKnownPort))
        .subscribe();
  ...
  private void initializeInetAddresses() {
    ...
    inetAddressMap = ImmutableMap.copyOf(Cd11ConnManUtil
        .buildAddressMap(cd11ConnManSystemConfiguration, DATA_MANAGER_ADDRESS_KEY,
            DATA_PROVIDER_KEY));
