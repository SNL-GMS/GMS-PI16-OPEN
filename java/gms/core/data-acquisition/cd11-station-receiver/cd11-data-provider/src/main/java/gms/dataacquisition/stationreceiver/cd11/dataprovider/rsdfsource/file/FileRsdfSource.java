...
public class FileRsdfSource implements RsdfSource {
  ...
  public Flux<RawStationDataFrame> getRsdfFlux() {
    ...
    final File seedDataDirectory = new File(dataLocation);
    if (!seedDataDirectory.exists() || !seedDataDirectory.isDirectory()) {
      ...
      Duration seedReferenceShift = Duration.between(referenceTime, now.plus(initialDelay));
  ...
  private byte[] modifyRawPayload(RawStationDataFrame rsdFrame, Instant newPayloadStart) {
    try {
      ...
      Cd11Data modifiedData = Cd11Data.builder()
          ...
          .build();
      Cd11Frame modifiedFrame = Cd11FrameFactory.createDefault()
          .wrap(modifiedData, dataFrame.getHeader().getSequenceNumber());
