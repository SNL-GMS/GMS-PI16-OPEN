...
class RsdfFileSourceModifier implements Modifier<Iterable<RawStationDataFrame>> {
  ...
  private final Cd11FrameFactory frameFactory = Cd11FrameFactory.createDefault();
  ...
  private byte[] modifyRawPayload(RawStationDataFrame rsdFrame, Instant payloadStartTime) {
    try {
      ...
      Cd11Data modifiedData = Cd11Data.builder()
          ...
          .build();
      final var modifiedFrame = frameFactory
          .wrap(modifiedData, currentRsdfInfoById.get(rsdFrame.getId()).getLeft());
