...
public class Cd11ClientFactory {
  ...
  private Cd11ClientFactory(Cd11FrameFactory frameFactory, Cd11ConnectionConfig connectionConfig) {
  ...
  public static Cd11ClientFactory create(String frameCreator, String frameDestination) {
    var frameFactory = Cd11FrameFactory.createUnauthenticated(frameCreator, frameDestination);
    var connectionConfig = Cd11ConnectionConfig.builder()
        .setProtocolMajorVersion((short) 1)
        .setProtocolMinorVersion((short) 1)
        .setStationOrResponderName("PROVIDER") //max 8 chars
        .setStationOrResponderType("IDC")
        .setServiceType("TCP")
        .build();

    return new Cd11ClientFactory(frameFactory, connectionConfig);
  }
  ...
  public Cd11Client createCd11Client(String stationName, Cd11ConnectionExchange connectionResponse,
    ...
    String host = InetAddresses.fromInteger(connectionResponse.getIpAddress()).toString();
    if (host.charAt(0) == '/') {
  ...
  public Cd11ConnectionExchange tryRequestConnection(String host, int port, String stationName)
    ...
    try (Socket client = getClientSocket(host, port)) {
      client.setSoTimeout(SOCKET_READ_TIMEOUT_MS);
      client.getOutputStream().write(frameFactory.wrapRequest(connectionRequest).toBytes());
      byte[] responseBuffer = new byte[1024];
      int bytesRead = client.getInputStream().read(responseBuffer);
      if (bytesRead == -1) {
