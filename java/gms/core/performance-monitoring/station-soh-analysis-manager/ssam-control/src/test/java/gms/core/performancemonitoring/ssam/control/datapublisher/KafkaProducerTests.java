...
class KafkaProducerTests {
  ...
  void testKafkaPublisher() {
    ...
    var sourceList = List.of(
        Instant.EPOCH,
        Instant.EPOCH.plusSeconds(10),
        Instant.EPOCH.plusSeconds(20)
    );

    var listCountDownLatch = new CountDownLatch(sourceList.size());
    ...
    var instantFlux = Flux.fromIterable(sourceList);

    var kafkaPublisher = KafkaProducer.<Instant>getBuilder()
        .reset()
        .withFlux(instantFlux)
        .withSender(kafkaSender)
        .withTopic(topic)
        .build();
    ...
    try {
      ...
      listCountDownLatch.await(1, TimeUnit.SECONDS);
  ...
  private static class MockKafkaSender<T> implements KafkaSender<String, String> {
    ...
    private final CountDownLatch listCountDownLatch;

    MockKafkaSender(
        Class<T> type,
        List<T> listToPopulate,
        String topic,
        CountDownLatch listCountDownLatch
    ) {
      this.type = type;
      this.listToPopulate = listToPopulate;
      this.topic = topic;
      this.listCountDownLatch = listCountDownLatch;
    }
