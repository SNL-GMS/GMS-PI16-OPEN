...
public class StationSohControl implements StationSohControlInterface {
  ...
  private static final StructuredLoggingWrapper logger = StructuredLoggingWrapper
      .create(LoggerFactory.getLogger(StationSohControl.class));
  ...
  public synchronized void start() {
    ...
    if (logger.getWrappedLogger().isInfoEnabled()) {
  ...
  public StationSohMonitoringResultsFluxPair monitor(
    ...
    final String extracts = "extracts";
          ...
          rollupFluxBuilder.getStationSohFlux()
              ...
              .doOnComplete(
                  () -> {
                    ...
                    logger.addKeyValueArgument("StationSohCompMsec", tempValue);
                    logger.addKeyValueArgument(extracts, acquiredStationSohExtracts.size());
                    logger.info("COMPLETED stationSoh flux!");
                    logger.removeArgument("StationSohCompMsec");
                    logger.removeArgument(extracts);
          ...
          rollupFluxBuilder.getCapabilitySohRollupFlux()
              ...
              .doOnComplete(
                  () -> {
                    ...
                    logger.addKeyValueArgument("CapSohRollupCompMsec", tempValue);
                    logger.addKeyValueArgument(extracts, acquiredStationSohExtracts.size());
                    logger.info("COMPLETED capability flux!");
                    logger.removeArgument("CapSohRollupCompMsec");
                    logger.removeArgument(extracts);
  ...
  private static String getSystemConfig(SystemConfig systemConfig, String key,
    ...
    try {
      ...
      if (logger.getWrappedLogger().isWarnEnabled()) {
