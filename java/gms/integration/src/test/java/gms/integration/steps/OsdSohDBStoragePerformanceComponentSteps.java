...
public class OsdSohDBStoragePerformanceComponentSteps<T> {
  ...
  public void iVerifyNumMessagesAreStoredInTheDBInOneSecond(int numMessages, String duration, String tableType) {
    ...
    switch (osdTableType) {
      ...
      case ACEI_BOOLEAN:
        for (List<T> myList : storageObjects) {
          environment.getSohRepositoryInterface().syncAceiUpdates(AceiUpdates.builder()
            .setBooleanInserts((Collection<AcquiredChannelEnvironmentIssueBoolean>) myList)
            .build());
        }
        break;
      case ACEI_ANALOG:
        for (List<T> myList : storageObjects) {
          environment.getSohRepositoryInterface().syncAceiUpdates(AceiUpdates.builder()
            .setAnalogInserts((Collection<AcquiredChannelEnvironmentIssueAnalog>) myList)
            .build());
        }
        break;
    }
    ...
    logger.info(osdTableType + "::avgTimeElapsed: " + avgTime);
