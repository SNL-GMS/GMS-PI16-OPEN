...
public class AceiQueryComponentSteps {
  ...
  private final List<AcquiredChannelEnvironmentIssue.AcquiredChannelEnvironmentIssueType> aceiTypes =
      new ArrayList<>();
...
//        this.environment.getSohRepositoryInterface().storeAcquiredChannelSohAnalog();
    ...
    historicalACEIList.forEach(
        hacei -> logger.info("{} : {} : {}", hacei.getChannelName(), hacei.getMonitorType(),
            hacei.getTrendLine()));

    // Save the ACEI bools and analogs to the OSD
    try {
      this.environment.getSohRepositoryInterface().syncAceiUpdates(
        AceiUpdates.builder()
          .setAnalogInserts(aceiAnalogs)
          .setBooleanInserts(aceiBools)
          .build()
      );
    } catch (Exception e) {
  ...
  public void retrieveAceiAnalogAndBooleanObjects() {
    ...
    queryHistoricalACEIList.forEach(
        hacei -> logger.info("{} : {} : {}", hacei.getChannelName(), hacei.getMonitorType(),
            hacei.getTrendLine()));
