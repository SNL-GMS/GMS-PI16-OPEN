...
public abstract class SignalDetection {
  ...
  public abstract SignalDetectionId getId();
  ...
  public static SignalDetection from(
    @JsonProperty("id") SignalDetectionId id,
    ...
    data.ifPresent(dataVal -> {
      for (SignalDetectionHypothesis sdh : dataVal.getSignalDetectionHypotheses()) {
        checkState(id.getId().equals(sdh.getId().getSignalDetectionId()),
          "SignalDetectionHypothesis with SignalDetectionId=%s " +
  ...
  public static SignalDetection createEntityReference(UUID id) {
    return new AutoValue_SignalDetection.Builder()
      .setId(SignalDetectionId.from(id))
      .build();
  }
  ...
  public interface Builder {
    SignalDetection.Builder setId(SignalDetectionId id);
      ...
      public SignalDetection.Data build() {
        ...
        if (numPresentFields == 0) {
          ...
          if (numHypothesesPopulated == data.getSignalDetectionHypotheses().size()) {
            ...
            for (int i = 0; i < data.getSignalDetectionHypotheses().size(); i++) {
              ...
              if (possibleParentID.isPresent()) {
                checkState(hypothesisIds.contains(possibleParentID.get()),
                  "Cannot create a SignalDetection with a hypothesis whose parent is not contained in the detection");
                checkState(hypothesisIds.indexOf(possibleParentID.get()) < i,
  ...
  public SignalDetection reject(UUID signalDetectionHypothesisIdToClone) {
    ...
    SignalDetectionHypothesis.Data sdhData = sdh.getData().orElseThrow().toBuilder().setRejected(true)
      .setParentSignalDetectionHypothesisId(Optional.ofNullable(sdh.getId().getId())).build();
    SignalDetectionHypothesis sdh2 = SignalDetectionHypothesis.from(
      SignalDetectionHypothesisId.from(sdh.getId().getSignalDetectionId(), UUID.randomUUID()),
      Optional.of(sdhData));

    SignalDetection.Data.Builder builder = getDataOrThrow().toBuilder();
    ...
    SignalDetection.Data data = builder.setSignalDetectionHypotheses(originalHypotheses).build();
