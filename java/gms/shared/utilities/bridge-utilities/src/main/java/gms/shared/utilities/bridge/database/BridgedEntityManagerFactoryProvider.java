...
public class BridgedEntityManagerFactoryProvider {
  ...
  private static final StructuredLoggingWrapper logger = StructuredLoggingWrapper
    .create(LoggerFactory.getLogger(BridgedEntityManagerFactoryProvider.class));
  ...
  private static final String WALLET_BASE_PATH = "*/gms/";
  ...
  public EntityManagerFactory getEntityManagerFactory(String unitName, String jdbcUrl, SystemConfig config) {
    ...
    if (!BridgedEntityManagerFactoryProvider
      .walletExists(System.getProperty(WALLET_LOCATION_PROPERTY_KEY))) {
  ...
  private static boolean walletExists(String walletPath) {

    if (PathValidation.validateDirectoryPath(walletPath, WALLET_BASE_PATH)) {
      logger.warn("Path manipulation detected, returning wallet does not exist.");
      return false;
    }

    return Files.exists(Path.of(walletPath))
      && Files.exists(Path.of(walletPath, "cwallet.sso"))
      && Files.exists(Path.of(walletPath, "ewallet.p12"));
  }
}
