...
class IntervalConverter {
  ...
  IntervalConverter(AutomaticIntervalConverter automaticIntervalConverter,
      InteractiveIntervalConverter interactiveIntervalConverter, Map<String, Stage> stagesByName) {
    this.automaticIntervalConverter = automaticIntervalConverter;
    this.interactiveIntervalConverter = interactiveIntervalConverter;
    this.stagesByName = stagesByName;
  }

  public static IntervalConverter create(Workflow workflow) {
    Map<String, Stage> stagesByName = workflow.getStages().stream()
      .collect(Collectors.toMap(Stage::getName, identity()));

    return new IntervalConverter(new AutomaticIntervalConverter(),
      new InteractiveIntervalConverter(), stagesByName);
  }

  /**
   * Attempts to convert a legacy {@link IntervalDao} into a COI {@link StageInterval} using known conversions from
   * the legacy stages to configured system {@link Stage}s. The kind of StageInterval generated is dependent on the
   * kind of Stage the IntervalDao comes from.
   *
   ...
   * expected interval time ranges are provided.
   * Ordering will be respected as best as possible in the return based on the input ordering of the IntervalDaos, given
   * the grouping by Stage that occurs.
   *
   ...
   */
  public Map<String, List<StageInterval>> convert(List<IntervalDao> intervalDaos, Instant queryStart, Instant queryEnd) {
  ...
  /**
   * Predicate generator for filtering out {@link IntervalDao}s that do not enclose the input time range. Used to
   * narrow down the possible IntervalDaos we'll use to generate a Auto Network {@link StageInterval}
   *
