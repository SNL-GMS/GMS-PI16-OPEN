...
public class ServiceClientJdkHttp {
  ...
  public <T> T send(ServiceRequest request) {
    if(!upgradedToHTTP2) {
      Instant upgradeStart = Instant.now();
      upgradedToHTTP2 = requestSucceeded(throwIfErrorResponse(performHTTP2Upgrade(request)));
      Instant upgradeEnd = Instant.now();
      LOGGER.debug("Upgrading to http2 took {} ms", Duration.between(upgradeStart, upgradeEnd).toMillis());
    }
  ...
  private static <F> F trySerialize(
    ...
    try {
      Instant serializationStart = Instant.now();
      var serialized = requestProtocol.serialize(data);
      Instant serializationEnd = Instant.now();
      LOGGER.debug("Serialization took {} ms", Duration.between(serializationStart, serializationEnd).toMillis());
      return serialized;
    } catch (Exception ex) {
  ...
  private HttpResponse<String> performHTTP2Upgrade(ServiceRequest request) {
    try {
      ...
      return Failsafe.with(retryPolicy).get(() -> httpClient.send(httpRequest, BodyHandlers.ofString()));
    } catch (Exception e) {
  ...
  private static <T, F> T tryDeserialize(
      ...
      Type type) {
    try {
      Instant deserializationStart = Instant.now();
      T deserialized = responseProtocol.deserialize(data, type);
      Instant deserializationEnd = Instant.now();
      LOGGER.debug("Deserialization took {} ms", Duration.between(deserializationStart, deserializationEnd).toMillis());
      return deserialized;
    } catch (Exception ex) {
