...
import gms.shared.frameworks.osd.coi.channel.*;
...
@Testcontainers
public class StationAndChannelIntegrationTest extends SohDbTest {
  ...
  void testUpdateStationGroupsReplaceStation() {
    ...
    assertEquals(TestFixtures.STATION_GROUP.getName(), updatedStationGroup.getName());
    assertEquals(updatedGroupDefinition.getDescription(), updatedStationGroup.getDescription());
    assertNotEquals(STATIONS, updatedStationGroup.getStations());
  ...
  void testUpdateStationGroupsMissingStation() {
    ...
    assertThrows(IllegalArgumentException.class,
        () -> stationGroupRepository.updateStationGroups(List.of(newDefinitionMissingStation)));
  }
}
