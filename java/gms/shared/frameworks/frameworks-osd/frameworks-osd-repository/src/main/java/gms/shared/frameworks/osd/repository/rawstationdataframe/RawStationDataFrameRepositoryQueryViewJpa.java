...
public class RawStationDataFrameRepositoryQueryViewJpa implements RawStationDataFrameRepositoryQueryInterface {
  ...
  public Map<String, Instant> retrieveLatestSampleTimeByChannel(List<String> channelNames) {
    ...
    try {
      ...
      latestSampleTimeQuery.multiselect(fromWaveformSummary.get("channelName").alias("channelName"),
          builder.max(fromWaveformSummary.get("endTime")).alias("endTime"))
          .where(fromWaveformSummary.get("channelName").in(channelNames))
          .groupBy(fromWaveformSummary.get("channelName"));

      return entityManager.createQuery(latestSampleTimeQuery).getResultStream()
        .map(tuple -> Pair.of(tuple.get("channelName", String.class), tuple.get("endTime", Instant.class)))
        .distinct()
