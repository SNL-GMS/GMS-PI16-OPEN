...
public class SystemConfig {
  ...
  private SystemConfig(String componentName, List<SystemConfigRepository> repositories) {
    ...
    logger.info(
        "system-configuration: creating system configuration for component named {}", componentName);

    this.componentName = componentName;
    this.repositories = Collections.unmodifiableList(repositories);
    try {
      Level logLevel = Level.getLevel(getValue("log_level"));
      ...
      if(logLevel == null){
        logger.warn("The specified LogLevel \"{}\" has not been defined in the log4j.xml configuration file.  " +
            "Defaulting to the level defined in log4j.xml file", getValue("log_level"));
      }else {
  ...
  public Path getValueAsPath(String key) {
    ...
    String pathString = getValue(key);

    if (!PathValidation.validateDirectoryPath(pathString, "/rsdf")) {
      throw new IllegalArgumentException(String.format("Unexpected base path in %s", pathString));
    }
