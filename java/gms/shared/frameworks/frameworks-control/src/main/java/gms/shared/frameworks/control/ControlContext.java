...
public abstract class ControlContext {
  ...
  public abstract static class Builder {
    ...
    public ControlContext build() {
      SystemConfig systemConfig = getSystemConfigOrMakeDefault();
      final ConfigurationRepository configRepo = getConfigRepoOrMakeDefault();
      final RetryConfig retryConfig= RetryConfig.create(systemConfig.getValueAsInt("processing-retry-initial-delay"),systemConfig.getValueAsInt("processing-retry-max-delay"),
      ...
      processingConfigurationConsumerUtility(ConfigurationConsumerUtility
          .builder(configRepo)
    ...
    private ConfigurationRepository getConfigRepoOrMakeDefault() {
      ...
      if (!setConfigRepo.isPresent()) {
        configRepo = ClientGenerator.createClient(ConfigurationRepository.class);
