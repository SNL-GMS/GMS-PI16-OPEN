...
public class HttpService {
  ...
  private Optional<byte[]> serializeObjectMsgPack(Object obj) {
    try {
      ...
      byte[] serialized = definition.getMsgpackMapper().writeValueAsBytes(obj);
      Instant serializationEnd = Instant.now();
      logger.info("Serialization to msgpack took {} ms", Duration.between(serializationStart, serializationEnd).toMillis());
  ...
  private Optional<String> serializeObjectJson(Object obj) {
    try {
      Instant serializationStart = Instant.now();
      String serialized = definition.getJsonMapper().writeValueAsString(obj) + " ";
      Instant serializationEnd = Instant.now();
      logger.info("Serialization to json took {} ms", Duration.between(serializationStart, serializationEnd).toMillis());
      return Optional.of(serialized);
    } catch (IOException e) {
  ...
  private Publisher<Void> sendJsonResponse(HttpServerResponse response, Object body) {
    ...
    try {
      final String data = jsonMapper.writeValueAsString(body);
      return response
