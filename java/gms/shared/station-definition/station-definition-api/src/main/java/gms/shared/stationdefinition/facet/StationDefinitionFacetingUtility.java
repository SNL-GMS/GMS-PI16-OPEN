...
public class StationDefinitionFacetingUtility {
   ...
   * @return
   */
  public static StationDefinitionFacetingUtility create(StationDefinitionAccessorInterface stationDefinitionAccessorInterface) {
  ...
  private StationGroup getPopulatedStationGroup(StationGroup initial,
    ...
    final FacetingDefinition stationFacetingDefinition = facetingDefinition
      .getFacetingDefinitionByName(STATIONS_KEY.getValue());
    final StationGroup.Data data = initial.getData().orElseThrow();
  ...
  private Station populateChannelGroupsAndChannels(Station station, FacetingDefinition facetingDefinition,
    ...
    final FacetingDefinition channelGroupFacetingDefinition =
      facetingDefinition.getFacetingDefinitionByName(CHANNEL_GROUPS_KEY.getValue());
    final FacetingDefinition channelFacetingDefinition =
      facetingDefinition.getFacetingDefinitionByName(CHANNELS_KEY.getValue());

    final Station.Data data = station.getData().orElseThrow();
    ...
    // NOTE: The populated ChannelList may not contain the elements in the ChannelGroup.ChannelList
    Collection<String> channelNames = channelList.stream()
      .map(Channel::getName)
      .collect(Collectors.toList());
    ...
    if (channelList.isEmpty() || updatedChannelGroups.isEmpty()) {
      return null;
    } else {
      return station.toBuilder()
        .setData(data.toBuilder()
          .setAllRawChannels(channelList)
  ...
  private ChannelGroup populateChannels(ChannelGroup channelGroup, FacetingDefinition facetingDefinition,
    ...
    final FacetingDefinition channelFacetingDefinition = facetingDefinition
      .getFacetingDefinitionByName(CHANNELS_KEY.getValue());

    final ChannelGroup.Data data = channelGroup.getData().orElseThrow();
  ...
  private Channel populateResponses(Channel channel, FacetingDefinition facetingDefinition,
    ...
    final FacetingDefinition responseFacetingDefinition = facetingDefinition
      .getFacetingDefinitionByName(RESPONSES_KEY.getValue());

    final Channel.Data data = channel.getData().orElseThrow();
    if (data.getResponse().isEmpty()) {
      return channel;
    }

    Response response = data.getResponse().get();
