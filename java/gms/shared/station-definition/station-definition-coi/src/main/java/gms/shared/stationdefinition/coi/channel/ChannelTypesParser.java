...
public class ChannelTypesParser {
  ...
  private static final Set<String> NON_CONVENTIONAL_INFRASOUND_CHANNELS = ImmutableSet.of("LDA", "BDA");
  ...
  private static Optional<ChannelTypes> handleThreeCharacterChannel(char bandCode,
    ...
    String chan = new StringBuilder().append(bandCode).append(instrumentCode).append(orientationCode).toString();
    ...
    ChannelBandType bandType = ChannelBandType.fromCode(bandCode);
    ChannelInstrumentType instrumentType = ChannelInstrumentType.fromCode(instrumentCode);
    ...
    ChannelDataType dataType = dataTypeOptional.get();
    ChannelTypes.Builder typesBuilder = ChannelTypes.builder()
      .setBandType(bandType)
      .setInstrumentType(instrumentType)
      .setOrientationCode(corectedOrientationCode)
      .setDataType(dataType);

    final ChannelOrientationType orientationType =
