...
public class DaoChannelConverter implements ChannelConverter {
  ...
  /**
   * creates and validates a new {@link DaoChannelConverter}
   *
   * @param calibrationConverter the provided {@link DaoCalibrationConverter}
   * @param fapConverter the provide {@link FileFrequencyAmplitudePhaseConverter}
   * @return a {@link DaoChannelConverter}
   */
  public static DaoChannelConverter create(
    DaoCalibrationConverter calibrationConverter,
    FileFrequencyAmplitudePhaseConverter fapConverter) {
    Objects.requireNonNull(calibrationConverter);
    Objects.requireNonNull(fapConverter);
    return new DaoChannelConverter(calibrationConverter, fapConverter);
  }
  ...
  public Channel convert(SiteChanDao siteChanDao,
    ...
    Objects.requireNonNull(sensorDao, "Sensor must not be null.");
    ...
    Response response = wfdiscDao == null ?
      null :
      getResponse(sensorDao, wfdiscDao, instrumentDao, responseConverterTransform);
  ...
  private Response getResponse(SensorDao sensorDao, WfdiscDao wfdiscDao,
    ...
    if (instrumentDao == null) {
      ...
      var frequencyAmplitudePhase = fapConverter.convertToEntityReference(
        Path.of(instrumentDao.getDirectory(), instrumentDao.getDataFile()).toString());
