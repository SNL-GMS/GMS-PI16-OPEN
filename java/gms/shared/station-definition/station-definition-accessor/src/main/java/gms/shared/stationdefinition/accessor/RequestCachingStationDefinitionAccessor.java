...
public class RequestCachingStationDefinitionAccessor implements StationDefinitionAccessorInterface {
  ...
  private static final StructuredLoggingWrapper logger = StructuredLoggingWrapper.create(
    LoggerFactory.getLogger(RequestCachingStationDefinitionAccessor.class));
  ...
  private RequestCachingStationDefinitionAccessor(StationDefinitionAccessorInterface delegate,
  ...
  /**
   * Creates a {@link RequestCachingStationDefinitionAccessor} from the provided {@link RequestCache}
   * and {@link StationDefinitionAccessorInterface}
   *
   * @param delegate The next member of the chain of command that will be called on a cache miss
   * @param requestCache The cache that will store requests
   * @return {@link StationDefinitionAccessorInterface}
   */
  public static RequestCachingStationDefinitionAccessor create(StationDefinitionAccessorInterface delegate,
    RequestCache requestCache) {
    Preconditions.checkNotNull(delegate, "Delegate must not be null");
    Preconditions.checkNotNull(requestCache, "RequestCache must not be null");
    return new RequestCachingStationDefinitionAccessor(delegate, requestCache);
  }
  ...
  public List<StationGroup> findStationGroupsByNameAndTime(List<String> stationGroupNames, Instant effectiveAt) {
    ...
    if (cacheResponse.isEmpty()) {
      ...
      return cacheResponse.stream()
        .filter(obj -> obj instanceof StationGroup)
        .map(obj -> (StationGroup) obj)
  ...
  public List<StationGroup> findStationGroupsByNameAndTimeRange(List<String> stationGroupNames, Instant startTime,
    Instant endTime) {
    TimeRangeRequest timeRangeRequest = TimeRangeRequest.builder()
  ...
  public List<Station> findStationsByNameAndTime(List<String> stationNames, Instant effectiveTime) {
    ...
    if (cacheResponse.isEmpty()) {
      ...
      return cacheResponse.stream()
        .filter(obj -> obj instanceof Station)
        .map(obj -> (Station) obj)
        .collect(Collectors.toList());
    }
  }
  ...
  public List<Instant> determineStationChangeTimes(Station station, Instant startTime, Instant endTime) {
    StationChangeTimesRequest request = StationChangeTimesRequest.create(station, startTime, endTime);
    Collection<Object> cacheResponse = requestCache.retrieve(request);
    if (cacheResponse.isEmpty()) {
      List<Instant> delegateReponse = delegate.determineStationChangeTimes(station, startTime, endTime);
      requestCache.put(request, new ArrayList<>(delegateReponse));
      return delegateReponse;
    } else {
      return cacheResponse.stream()
        .filter(obj -> obj instanceof Instant)
        .map(obj -> (Instant) obj)
        .collect(Collectors.toList());
    }
  }
  ...
  public List<ChannelGroup> findChannelGroupsByNameAndTime(List<String> channelGroupNames, Instant effectiveAt) {
    ...
    if (cacheResponse.isEmpty()) {
      ...
      return cacheResponse.stream()
        .filter(obj -> obj instanceof ChannelGroup)
        .map(obj -> (ChannelGroup) obj)
        .collect(Collectors.toList());
    }
  }
  ...
  public Channel loadChannelFromWfdisc(long wfid, TagName associatedRecordType, long associatedRecordId,
    ...
    return delegate.loadChannelFromWfdisc(wfid, associatedRecordType, associatedRecordId, channelEffectiveTime, channelEndTime);
  }
  ...
  public List<Response> findResponsesById(Collection<UUID> reponseIds, Instant effectiveTime) {
    ...
    if (cacheResponse.isEmpty()) {
      ...
      return cacheResponse.stream()
        .filter(obj -> obj instanceof Response)
