...
Or:
    image: {{ include "ian.container.image" (dict "Values" .Values "appValues" (dict "imageName" "gms-common/centos")) }}
*/}}
{{- define "ian.container.image" -}}
  {{- if contains ":" .appValues.imageName -}}
    {{ printf "%s/%s" .Values.imageRegistry .appValues.imageName | quote }}
  {{- else -}}
    {{ printf "%s/%s:%s" .Values.imageRegistry .appValues.imageName (include "ian.imageTag" .) | quote }}
  {{- end -}}
{{- end -}}
...
Usage:
          env: {{- include "ian.app.deployment.env" (dict "appName" $appName "appEnv" $appValues.env "globalEnv" .Values.env "context" $) }}
*/}}
{{- define "ian.app.deployment.env" -}}
...
*/}}
{{- define "ian.deployment.strategy" -}}
...
*/}}
{{- define "ian.container.initContainer.wait-for-config-load" -}}
...
*/}}
{{- define "ian.container.resources" -}}
