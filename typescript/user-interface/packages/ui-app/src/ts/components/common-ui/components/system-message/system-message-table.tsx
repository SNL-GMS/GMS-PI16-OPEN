...
> {
  ...
  private readonly autoScrollTimer: NodeJS.Timeout;
  ...
  public static getDerivedStateFromProps(
    ...
    if (nextProps.systemMessages?.length && !isScrolledToLatest && messagesHaveChanged) {
      ...
      stateModifications.hasUnseenMessages = true;
      ...
      if (nextProps.systemMessages.length === 0) {
        stateModifications.hasUnseenMessages = false;
      }
    }
  ...
  public shouldComponentUpdate(
    ...
    const shouldItUpdate =
      messagesHaveChanged ||
      nextProps.isAutoScrollingEnabled !== this.props.isAutoScrollingEnabled ||
      this.state.hasUnseenMessages !== nextState.hasUnseenMessages;
  ...
  public componentDidUpdate(
    ...
    if (
      ...
      this.setState({ isScrolledToLatest: this.isScrolledToLatest() });
    }
  ...
  private readonly sortByTime = (order: 'asc' | 'desc' = 'asc') => {
    if (this.table.getTableApi()) {
      this.table.getTableApi().setSortModel([
        {
          colId: 'time',
          sort: order
        }
  ...
  private readonly scrollToLatest = () => {
    this.setState({ hasUnseenMessages: false, isScrolledToLatest: true });
    if (this.state.systemMessages) {
      ...
      setTimeout(
        ...
        this.SCROLL_TO_VISIBLE_TIMEOUT_MS
      );
    }
  };
    ...
    if (this.state.systemMessages?.length) {
      if (this.getScrollDirection() === 'up') {
        if (this.state.hasUnseenMessages) {
          this.setState({ hasUnseenMessages: !this.isScrolledToLatest() });
        }
        this.props.setIsAutoScrollingEnabled(false);
      }
    }
   ...
   */
  private readonly getSortOrderForTime = (): 'asc' | 'desc' | undefined => {
    const model = this.table?.getTableApi()?.getSortModel();
    const timestampModel = model && model.length > 0 && model.find(m => m.colId === 'time');
    return timestampModel ? (timestampModel.sort as 'asc' | 'desc') : undefined;
  };
