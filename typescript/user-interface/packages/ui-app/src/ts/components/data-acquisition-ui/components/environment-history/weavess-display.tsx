...
export const WeavessDisplay: React.FunctionComponent<
  EnvironmentHistoryPanelProps & {
    startTimeMs: number;
    endTimeMs: number;
  }
> = props => {
  const context = React.useContext(ApolloClientInterface.HistoricalAceiQueryContext);
  ...
  return (
    ...
    >
      <div className="weavess-container__wrapper">
        <Weavess
          startTimeSecs={props.startTimeMs / MILLISECONDS_IN_SECOND}
          endTimeSecs={props.endTimeMs / MILLISECONDS_IN_SECOND}
          stations={stations}
          selections={{
            channels: undefined
          }}
