...
import { AxiosClientInterface } from '~components/data-acquisition-ui/client-interface';
...
import { HistoricalTrendsInnerPanelProps, HistoricalTrendsPanelProps } from './types';
 ...
 */
const HistoricalTrendsHistoryInnerPanel: React.FunctionComponent<
  HistoricalTrendsPanelProps & HistoricalTrendsInnerPanelProps
> = props => {
  ...
  return (
    <>
      <BaseToolbar
        ...
        itemsLeft={[legend]}
        items={[props.timeIntervalSelector]}
      />
      <DrillDownTitle
 ...
 * @param props the props
 */
export const HistoricalTrendsHistoryPanel: React.FunctionComponent<HistoricalTrendsPanelProps> = props => {
  ...
  const configuration = AxiosClientInterface.Queries.SohConfigurationQuery.useSohConfigurationQuery();

  // request historical SOH data;
  const historicalSohByStation = AxiosClientInterface.Queries.HistoricalSoh.useHistoricalSohByStationQuery(
    {
      samplesPerChannel: configuration.data.historicalSamplesPerChannel,
      stationName: props.station.stationName,
      startTime: startTimeMs,
      endTime: endTimeMs,
      sohMonitorType: props.monitorType
    },
    configuration.data.maxHistoricalQueryIntervalSizeMs
  );

  const nonIdealState = validateNonIdealState(
    props.monitorType,
    historicalSohByStation.isLoading,
  ...
  if (nonIdealState) {
    return (
      <>
        <BaseToolbar widthPx={widthPx} items={[timeIntervalSelector]} />
