import { CommonTypes, UserProfileTypes, WorkflowTypes } from '@gms/common-model';
import { KeyValue } from '@gms/common-util';
import GoldenLayout, { ReactComponentConfig } from '@gms/golden-layout';
import { CommonWorkspaceTypes, UserSessionTypes } from '@gms/ui-state';
...
export interface GoldenLayoutPanelProps {
  ...
  setGlDisplayState(displayName: string, displayState: CommonWorkspaceTypes.GLDisplayState);
  setLayout(newLayout: {
...
export interface GoldenLayoutComponentBaseProps {
  ...
  setAuthStatus(auth: UserSessionTypes.AuthStatus): UserSessionTypes.AuthStatus;
}
