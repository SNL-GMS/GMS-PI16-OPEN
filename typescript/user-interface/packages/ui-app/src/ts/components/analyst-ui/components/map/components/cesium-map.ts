...
const { LEFT_DOUBLE_CLICK } = Cesium.ScreenSpaceEventType;

/**
 * Implementation of MapAPI Interface
 */
export class CesiumMap implements MapAPI {
   ...
   */
  public initialize = (containerDomElement: HTMLDivElement): void => {
   ...
   */
  public getViewer = (): any => this.viewer;
  ...
  public drawUnAssociatedSignalDetections = (
    ...
    UnAssociatedRenderer.draw(
      this.state.layers.UnAssociated,
      signalDetections,
      events,
      currentlyOpenEvent,
      defaultStations
   ...
   */
  public highlightSelectedSignalDetections(selectedSignalDetections: string[]): void {
    ...
    UnAssociatedRenderer.highlightSelectedSignalDetections(
      this.state.layers.UnAssociated,
      selectedSignalDetections
    );
  }
   ...
   */
  public drawDefaultStations = (
    currentDefaultStations: ProcessingStationTypes.ProcessingStation[],
    nextdefaultStations: ProcessingStationTypes.ProcessingStation[]
  ): void => {
   ...
   */
  public drawEvents(currentProps: MapProps, nextProps: MapProps): void {
  ...
  public highlightOpenEvent = (
    ...
    if (nextOpenEvent) {
      const loc =
        nextOpenEvent.currentEventHypothesis.eventHypothesis.preferredLocationSolution
          .locationSolution;
      const currentCameraHeight = this.viewer.camera.positionCartographic.height;
      this.viewer.camera.flyTo({
        destination: Cesium.Cartesian3.fromDegrees(
          loc.location.longitudeDegrees,
          loc.location.latitudeDegrees,
          currentCameraHeight
        ),
        duration: 2
      });
  ...
  private readonly createMapViewer = (containerElement: any) => {
    ...
    if (this.options.analystUiConfig.environment.map.online) {
      imageryProviderViewModels.push(
        new Cesium.ProviderViewModel({
    ...
    const viewerSettings = {
      ...
      selectedImageryProviderViewModel: imageryProviderViewModels[selectedImageryProviderIndex],
      geocoder: false
    };
   ...
   */
  private readonly setupLayers = (viewer: any): void => {
   ...
   */
  private readonly setupFrameRateMonitor = (viewer: any): void => {
    const frameRateMonitor = Cesium.FrameRateMonitor.fromScene(viewer.scene);
    // eslint-disable-next-line
    const oneHundredPercentResolutionScale = 1;
    ...
    frameRateMonitor.lowFrameRate.addEventListener(() => {
      UILogger.Instance().info('Low FPS - Lowering Resolution Scale');
    ...
    frameRateMonitor.nominalFrameRate.addEventListener(() => {
      if (viewer.resolutionScale < oneHundredPercentResolutionScale) {
        UILogger.Instance().info('Nominal FPS - Restoring Resolution');
        // eslint-disable-next-line no-param-reassign
   ...
   */
  private readonly onMapClick = (e: any): void => {
   ...
   */
  private readonly onMapRightClick = (e: any): void => {
    const entityWrapper = this.viewer.scene.pick(e.position);
    if (Cesium.defined(entityWrapper)) {
   ...
   */
  private readonly onMapCtrlClick = (e: any): void => {
    const entityWrapper = this.viewer.scene.pick(e.position);
    if (Cesium.defined(entityWrapper)) {
   ...
   */
  private readonly onMapAltClick = (e: any): void => {
    const entityWrapper = this.viewer.scene.pick(e.position);
    if (Cesium.defined(entityWrapper)) {
   ...
   */
  private readonly onMapDoubleClick = (e: any): void => {
    const entityWrapper = this.viewer.scene.pick(e.position);
    if (Cesium.defined(entityWrapper)) {
