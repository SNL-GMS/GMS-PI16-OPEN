...
export class FkDisplay extends React.Component<FkDisplayProps, FkDisplayState> {
  ...
  public render(): JSX.Element {
    ...
    if (this.props.multipleSelected || !signalDetection) {
      ...
      const message = this.props.multipleSelected
        ? 'Multiple FKs Selected'
        : this.props.anySelected
        ? 'No FK data for selected SD'
        : 'No SD selected';

      const icon = this.props.multipleSelected ? IconNames.MULTI_SELECT : IconNames.HEAT_GRID;

      return <NonIdealState icon={icon} title={message} />;
    }
    const selectedFk = signalDetection
      ? getFkData(signalDetection.currentHypothesis.featureMeasurements)
      : undefined;
    const fstatData = selectedFk ? selectedFk.fstatData : undefined;
    const selectedSd = !signalDetection ? undefined : signalDetection;
    ...
    const fkWindow: ToolbarTypes.LabelValueItem = {
    ...
    const presetWindowDropdown: ToolbarTypes.DropdownItem = {
    ...
    const leadInput: ToolbarTypes.NumericInputItem = {
    ...
    const durationInput: ToolbarTypes.NumericInputItem = {
      type: ToolbarTypes.ToolbarItemType.NumericInput,
      rank: 4,
    ...
    const stepInput: ToolbarTypes.NumericInputItem = {
      type: ToolbarTypes.ToolbarItemType.NumericInput,
      rank: 5,
    ...
    const fkLoadingSpinner: ToolbarTypes.LoadingSpinnerItem = {
  ...
  private readonly onLeadTimeChanged = (newLeadSeconds: number) => {
    ...
    const priorConfig = getFkData(this.props.signalDetection.currentHypothesis.featureMeasurements)
      .configuration;
