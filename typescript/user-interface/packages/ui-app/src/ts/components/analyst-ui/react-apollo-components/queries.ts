  ...
  return graphql(EventQueries.eventsInTimeRangeQuery, {
    options: (props: T) => {
      const variables: EventTypes.EventsInTimeRangeQueryArgs = {
        timeRange: systemConfig.getEventsTimeRange(props.currentTimeInterval, props.analysisMode)
      };
      return {
        variables,
...
export function graphqlQcMasksByChannelNameQuery<
  T extends BaseProps & GQLTypes.DefaultStationsQueryProps
>() {
  return graphql(QcMaskQueries.qcMasksByChannelNameQuery, {
    options: (props: T) => {
      ...
      const variables: QcMaskTypes.QcMasksByChannelNameQueryArgs = {
        timeRange: systemConfig.getDefaultTimeRange(props.currentTimeInterval, props.analysisMode),
        channelNames
      };
      return {
        variables
