...
export interface SignalDetectionContextMenuProps {
  ...
  changeAssociation: MutationFunction;
  associateToNewEvent: MutationFunction;
...
> {
  ...
  public render(): JSX.Element {
    ...
    const anyInConflictAndNotAssociatedToOpenEvent: boolean = SignalDetectionUtils.isInConflictAndNotAssociatedToOpenEvent(
      this.props.selectedSds,
      this.props.currentOpenEvent
    );
    const anyInConflict = SignalDetectionUtils.isInConflict(this.props.selectedSds);
            ...
            <MenuItem
              ...
              onClick={() => {
                ...
                updatedEntries.forEach((value, key) => {
                  updatedEntries = updatedEntries.set(key, false);
                });
  ...
  private readonly eventAssociationContextMenu = (
    ...
    const associatedInList: boolean =
      sdHypotheses.filter(sdHyp =>
        SignalDetectionUtils.isAssociatedToCurrentEventHypothesis(sdHyp, event)
      ).length > 0;
    const unassociatedInList: boolean =
      sdHypotheses.filter(
        sdHyp => !SignalDetectionUtils.isAssociatedToCurrentEventHypothesis(sdHyp, event)
      ).length > 0;
    const menuOptions = [];
    menuOptions.push(
      <MenuItem
            ...
            <MenuItem
              ...
              onClick={() => {
                const sdIdList = signalDetections
                  .filter(
                    sd =>
                      !SignalDetectionUtils.isAssociatedToCurrentEventHypothesis(
 ...
 */
export function setPhaseContextMenu(sdIds: string[], rePhaser: MutationFunction): JSX.Element {
  return (
    <PhaseSelectionMenu
...
function rePhaseDetections(
  ...
  detectionRephaser: MutationFunction
) {
