...
import { IconName, Intent, NonIdealState, Spinner } from '@blueprintjs/core';
 ...
 */
export const TableInvalidState: React.FunctionComponent<TableInvalidStateProps> = props => (
  <NonIdealState
    icon={props.visual}
    action={
      props.message === TableDataState.NO_SDS || props.message === TableDataState.NO_EVENTS ? (
        <Spinner intent={Intent.PRIMARY} />
      ) : null
    }
    title={
      props.message === TableDataState.NO_INTERVAL
        ? `No ${props.dataType}s loaded`
        : nonIdealStateMessages.title.get(props.message)
    }
