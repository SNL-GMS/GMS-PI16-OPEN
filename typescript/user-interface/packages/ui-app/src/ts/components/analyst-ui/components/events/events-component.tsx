...
import { CommonTypes, EventTypes, SignalDetectionTypes, WorkflowTypes } from '@gms/common-model';
...
export class Events extends React.Component<EventsProps, EventsState> {
  ...
  public static getDerivedStateFromProps(
    nextProps: EventsProps,
    prevState: EventsState
  ): {
    currentTimeInterval: CommonTypes.TimeRange;
  ...
  public componentDidUpdate(prevProps: EventsProps): void {
    if (
      ...
      ? this.props.eventsInTimeRangeQuery.eventsInTimeRange.filter(
          event =>
            event.currentEventHypothesis.eventHypothesis.preferredLocationSolution.locationSolution
              .location.time >= this.props.currentTimeInterval.startTimeSecs &&
            event.currentEventHypothesis.eventHypothesis.preferredLocationSolution.locationSolution
              .location.time <= this.props.currentTimeInterval.endTimeSecs
        ).length
      : 0;
  }
  ...
  private handleMarkSelectedComplete() {
    ...
    const { stageId } = selectedNodes[0].data;
  ...
  private readonly openNextEvent = () => {
    if (this.props.analysisMode === WorkflowTypes.AnalysisMode.EVENT_REVIEW) {
      defer(() => {
