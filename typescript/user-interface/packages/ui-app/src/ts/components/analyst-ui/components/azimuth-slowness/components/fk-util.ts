import { EventTypes, FkTypes, SignalDetectionTypes } from '@gms/common-model';
...
export const getPredictedPoint = (
  ...
  const azValue =
    predictedAzimuth && isSdInstantMeasurementValue(predictedAzimuth.predictedValue)
      ? predictedAzimuth.predictedValue
      : undefined;

  const predictedSlowness = featurePredictions.find(
    fp => fp.predictionType === SignalDetectionTypes.FeatureMeasurementTypeName.SLOWNESS
  );

  const slowValue =
    predictedSlowness && isSdInstantMeasurementValue(predictedSlowness.predictedValue)
      ? predictedSlowness.predictedValue
      : undefined;

  if (azValue && slowValue) {
    return {
      slowness: slowValue.value,
      slownessUncertainty: slowValue.standardDeviation,
      azimuth: azValue.value,
      azimuthUncertainty: azValue.standardDeviation,
      peakFStat: undefined
    };
  }
  return undefined;
...
export function drawMaxFk(
  ...
  if (!currentSpectrum) {
    ...
    console.log(`Undefined spectrum - index set to ${currentMovieSpectrumIndex}`);
  }
...
export function createFkInput(
  ...
  return {
    stationId: signalDetection.stationName,
...
export function getSortedSignalDetections(
  ...
  distanceToSource: EventTypes.LocationToStationDistance[]
  ...
  if (selectedSortType === AnalystWorkspaceTypes.WaveformSortType.distance && distanceToSource) {
    data = sortBy<SignalDetectionTypes.SignalDetection>(signalDetections, [
      (sd: SignalDetectionTypes.SignalDetection) =>
        distanceToSource.find(d => d.stationId === sd.stationName).distance
    ]);
  } else {
