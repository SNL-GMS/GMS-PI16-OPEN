 ...
 */
import { EventTypes, ProcessingStationTypes, SignalDetectionTypes } from '@gms/common-model';

import { userPreferences } from '~analyst-ui/config';
import { semanticColors } from '~scss-config/color-preferences';
// eslint-disable-next-line @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports, import/no-dynamic-require
...
const selectedStationColor = Cesium.Color.fromCssColorString(semanticColors.analystOpenEvent);
/**  Color to be used for stations */
const stationColor = Cesium.Color.fromCssColorString(semanticColors.mapStationsUnselected);
...
function addDefaultStations(
  ...
  defaultStations.forEach(defaultStation => {
    const siteLon = defaultStation.location.longitudeDegrees;
    const siteLat = defaultStation.location.latitudeDegrees;
    ...
    const stationSites = defaultStation.channelGroups;
    const siteStationID = defaultStation.name;

    stationSites.forEach(stationSite => {
      const stationSiteID: string = stationSite.name;
      const stationSiteLon = stationSite.location.longitudeDegrees;
      const stationSiteLat = stationSite.location.latitudeDegrees;
...
export function update(
  ...
  if (currentOpenEvent && currentSignalDetections) {
    ...
    currentSignalDetections
      .filter(sd =>
        eventSDAssoc.find(
          evtSDAccoc => evtSDAccoc.signalDetectionHypothesis.id === sd.currentHypothesis.id
        )
      )
      .forEach(signalDetection => {
        const eventEntity = dataSource.entities.getById(signalDetection.stationName);
  ...
  if (nextSignalDetections && nextOpenEvent) {
    const eventSDAssoc =
      nextOpenEvent.currentEventHypothesis.eventHypothesis.signalDetectionAssociations;
 ...
 */
export function resetView(defaultStations: any[], dataSource: any): void {
  defaultStations.forEach(station => {
    // eslint-disable-next-line no-param-reassign
    dataSource.entities.getById(station.name).billboard.color = stationColor;
  });
}
