...
import { FkTypes, SignalDetectionTypes, WaveformTypes } from '@gms/common-model';
import isEqual from 'lodash/isEqual';
import React from 'react';

import { SignalDetectionUtils } from '~analyst-ui/common/utils';
import { getFkData } from '~analyst-ui/common/utils/fk-utils';
import { getSignalDetectionBeams } from '~analyst-ui/common/utils/signal-detection-util';
...
import { AzimuthSlownessPanelProps, AzimuthSlownessPanelState, FkParams } from './types';
...
> {
  ...
  public constructor(props: AzimuthSlownessPanelProps) {
    ...
    if (props.displayedSignalDetection) {
      const arrivalTime = SignalDetectionUtils.findArrivalTimeFeatureMeasurementValue(
        props.displayedSignalDetection.currentHypothesis.featureMeasurements
      ).value;
  ...
  public componentDidUpdate(prevProps: AzimuthSlownessPanelProps): void {
    if (
      ...
      const arrivalTime = SignalDetectionUtils.findArrivalTimeFeatureMeasurementValue(
        this.props.displayedSignalDetection.currentHypothesis.featureMeasurements
      ).value;
  ...
  private readonly calculateMovieIndex = (time: number): number => {
    const signalDetectionBeam = getSignalDetectionBeams(
      [this.props.displayedSignalDetection],
      WaveformTypes.UNFILTERED_FILTER
    );
    if (signalDetectionBeam[0] === undefined) {
      return 0;
    }
    const { startTime } = signalDetectionBeam[0];
