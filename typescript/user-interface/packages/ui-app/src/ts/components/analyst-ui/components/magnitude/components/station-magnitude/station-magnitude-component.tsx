...
import { AmplitudesByStation, MagDefiningStates } from '../../types';
...
> {
  ...
  public render(): JSX.Element {
    ...
    const defStatesForMagnitudeType = new Map<EventTypes.MagnitudeType, MagDefiningStates>();
    Object.keys(EventTypes.MagnitudeType).forEach(magnitudeKey =>
      defStatesForMagnitudeType.set(
        EventTypes.MagnitudeType[magnitudeKey],
        this.getAllNoneDef(mainTableRowData, EventTypes.MagnitudeType[magnitudeKey])
      )
    );
    const stationIdsForMagnitudeType = new Map<EventTypes.MagnitudeType, string[]>();

    Object.keys(EventTypes.MagnitudeType).forEach(magTypeKey =>
      stationIdsForMagnitudeType.set(
        EventTypes.MagnitudeType[magTypeKey],
        mainTableRowData
          .filter(row => {
            const maybeMagData = row.dataForMagnitude.get(EventTypes.MagnitudeType[magTypeKey]);
   ...
   *
   * @returns a Map<EventTypes.MagnitudeType, MagnitudeDataForRow>
   */
  private readonly generateDataForMagnitude = (
    stationMagAndSignalDetection: StationMagAndSignalDetection
  ): Map<EventTypes.MagnitudeType, MagnitudeDataForRow> => {
    const dataPerMagnitude = new Map<EventTypes.MagnitudeType, MagnitudeDataForRow>();
    Object.keys(EventTypes.MagnitudeType).forEach(key => {
      const magnitudeType = EventTypes.MagnitudeType[key];
  ...
  private readonly generateStationMagAndSignalDetection = (
    ...
    const magnitudeTypeToAmplitudeData = new Map<EventTypes.MagnitudeType, MagnitudeAndSdData>();
    Object.keys(EventTypes.MagnitudeType).forEach(magKey => {
      magnitudeTypeToAmplitudeData.set(EventTypes.MagnitudeType[magKey], {
        magSolution: getNetworkMagSolution(
          this.props.locationSolution,
          EventTypes.MagnitudeType[magKey]
        ),
  ...
  private readonly getMagnitudeSolutionForStation = (
    magnitudeSolution: EventTypes.NetworkMagnitudeSolution,
    stationName: string
  ): EventTypes.NetworkMagnitudeBehavior =>
    magnitudeSolution
  ...
  private readonly onCellClicked = (params: any) => {
    if (
      ...
      const magType = params.column.colDef.magnitudeType as EventTypes.MagnitudeType;
      if (magType) {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly onShiftClick = (params: any, magType: EventTypes.MagnitudeType) => {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly onControlClick = (params: any, magType: EventTypes.MagnitudeType) => {
    const { data } = params;
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly onDefaultClick = (params: any, magType: EventTypes.MagnitudeType) => {
  ...
  private readonly getSelectedRowIndicesForPhase = (
    magType: EventTypes.MagnitudeType
  ): number[] => {
    const indices = [];
    if (this.mainTable) {
