...
import { AmplitudeBounds } from '../types';
...
export const calculateDataSegmentBounds = async (
  ...
  if (WeavessTypes.isDataClaimCheck(dataByClaim)) {
    ...
    const { max, min } = WeavessUtil.getBoundsForPositionBuffer(posBuffer, startIdx, endIdx);
...
/**
 * Takes a promise for bounds, and merges the resulting bounds with any stored bounds.
 */
const mergeWithExistingBounds = async (
  id: string,
  boundaryPromise: Promise<WeavessTypes.ChannelSegmentBoundaries>
): Promise<WeavessTypes.ChannelSegmentBoundaries> => {
  return mergeBounds(await BoundaryStore.retrieve(id), await boundaryPromise);
};

/**
 * Calculate the channel segment boundaries for the given channel segment, and store it in the
 * BoundaryStore for later retrieval.
 *
 * @param id the ID to use for storing these channel segment bounds
 * @param channelSegment the channel segment for which to calculate bounds
 */
export function calculateAndStoreBounds(
  id: string,
  channelSegment: WeavessTypes.ChannelSegment
): void {
  const boundaryPromise = calculateChannelSegBounds(channelSegment);
  if (BoundaryStore.has(id)) {
    BoundaryStore.store(id, mergeWithExistingBounds(id, boundaryPromise));
  } else {
    BoundaryStore.store(id, boundaryPromise);
  }
}
