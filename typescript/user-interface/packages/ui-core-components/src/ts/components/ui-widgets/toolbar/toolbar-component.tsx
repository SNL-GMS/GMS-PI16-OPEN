...
> {
   ...
   */
  public readonly hidePopup = (): void => ContextMenu.hide();
   ...
   */
  public readonly togglePopover = (rank: number): void => {
   ...
   */
  private readonly renderItem = (item: ToolbarTypes.ToolbarItem, hasIssue = false): JSX.Element => (
    <ToolbarItemRenderer
  ...
  private readonly handleResize = () => {
    ...
    if (overflowWidthPx > 0) {
      ...
      if (this.state.whiteSpaceAllotmentPx > reduceWhiteSpaceTo) {
        ...
        const { indicesToOverflow } = this.state;
        ...
        for (let i = this.toolbarItemRefs.length - 1; i >= 0; i -= 1) {
          ...
          overflowWidthPx -= item.getBoundingClientRect().width;
    ...
    } else if (overflowWidthPx < 0 && this.state.indicesToOverflow.length === 0) {
