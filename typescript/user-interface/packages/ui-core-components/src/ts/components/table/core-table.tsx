...
> {
   ...
   * while having the setting of `deltaRowDataMode` enabled.
   */
  private redrawRowNodesTimeout: NodeJS.Timeout;
  ...
  public readonly refreshRowNodesByIds = (ids: string[]): void => {
    if (this.tableApi && ids) {
      if (this.props.debug) {
        // eslint-disable-next-line no-console
        console.debug(`table refreshRowNodesByIds`);
      }
      ...
      this.tableApi.redrawRows({ rowNodes });
    }
  };
  ...
  public readonly refreshRenderedRowNodes = (): void => {
    if (this.tableApi) {
      if (this.props.debug) {
        // eslint-disable-next-line no-console
        console.debug(`table refreshRenderedRowNodes`);
      }
  ...
  public readonly refreshAllRowNodes = (): void => {
    if (this.tableApi) {
      if (this.props.debug) {
        // eslint-disable-next-line no-console
        console.debug(`table refreshAllRowNodes`);
      }
  ...
  public readonly setColumnDefinitions = (
    ...
    if (this.tableApi) {
      if (!isEqual(this.props.columnDefs, columnDefinitions)) {
        if (this.props.debug) {
          // eslint-disable-next-line no-console
          console.debug(`table setColumnDefinitions`);
        }
  ...
  /**
   * Default implementation of `getRowNodeId` for determine the unique
   * id of a row node in the table.
   *
   * @param data the data of the row
   */
  public readonly getRowNodeId = (data: RowDataType): string => {
    if (!data || data.id === undefined || data.id === null) {
      // eslint-disable-next-line no-console
      console.error(`ID undefined for row`);
    }
    return data.id;
  };
  ...
  private readonly onGridReady = (event: GridReadyEvent): void => {
    if (this.props.debug) {
      // eslint-disable-next-line no-console
      console.debug(`table onGridReady`);
    }
