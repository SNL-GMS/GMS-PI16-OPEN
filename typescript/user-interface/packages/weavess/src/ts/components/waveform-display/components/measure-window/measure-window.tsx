...
import { MeasureWindowProps } from './types';
  ...
  return React.useMemo(
    () => ({
      ...
      defaultChannel: {
        ...
        disableSignalDetectionModification: measureWindowSelection?.isDefaultChannel
          ? initialConfiguration?.defaultChannel?.disableSignalDetectionModification
          : initialConfiguration?.nonDefaultChannel?.disableSignalDetectionModification,
...
const useDefaultZoomWindow = (
  measureWindowSelection: WeavessTypes.MeasureWindowSelection | undefined
) =>
  React.useMemo(
    () => ({
      startTimeSecs: measureWindowSelection?.startTimeSecs ?? -1,
      endTimeSecs: measureWindowSelection?.endTimeSecs ?? -1
    }),
...
const MeasureWindow: React.FC<MeasureWindowProps> = (props: MeasureWindowProps) => {
  ...
  const defaultZoomWindow = useDefaultZoomWindow(props.measureWindowSelection);
  ...
  if (props.measureWindowSelection) {
    return (
      <WaveformPanel
        ...
        initialZoomWindow={defaultZoomWindow}
      />
    );
  }
  return (
