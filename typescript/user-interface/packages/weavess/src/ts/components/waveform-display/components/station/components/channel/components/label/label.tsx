...
export class Label extends React.PureComponent<WeavessTypes.LabelProps, LabelState> {
  ...
  public render(): JSX.Element {
    ...
    if (this.props.distance !== 0) {
      const fixBy = this.props.distanceUnits === WeavessTypes.DistanceUnits.degrees ? 1 : 2;
      distString += this.props.distance.toFixed(fixBy);
      distString += this.props.distanceUnits === WeavessTypes.DistanceUnits.km ? ' km' : '\u00B0';
    }
    return (
      <div
              ...
              {this.props.customLabel ? (
                ...
                <span>
                  ...
                  <div>
                    <p>
                      {distString}{' '}
                      <span className="label-container-content-mask-indicator" key={distString}>
  ...
  private readonly onContextMenu = (e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
    if (this.props.events?.onContextMenu) {
      if (e.button === 2 || (e.button === 0 && e.ctrlKey)) {
        e.preventDefault();
        this.props.events.onContextMenu(e, this.props.channelName, this.props.isDefaultChannel);
      }
    }
