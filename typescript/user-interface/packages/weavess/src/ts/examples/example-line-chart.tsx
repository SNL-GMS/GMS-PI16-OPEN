...
const generateDummyData = memoizeOne(
  (
      ...
      defaultChannel: {
        ...
        waveform: {
          ...
          channelSegments: new Map<string, WeavessTypes.ChannelSegment>([
            [
              'data',
              { channelName: 'ExampleChannel', wfFilterId: WeavessTypes.UNFILTERED, dataSegments }
            ]
          ])
        }
      },
    ...
    console.log(`Took ${(Date.now() - start) / 1000} seconds to generate data`);
...
> {
  ...
  private numberOfHoursTimeout: NodeJS.Timeout;

  private numberOfLinesTimeout: NodeJS.Timeout;

  private timeOffsetSecondsTimeout: NodeJS.Timeout;

  private pointFrequencyTimeout: NodeJS.Timeout;
  ...
  public componentDidUpdate(): void {
    if (this.state.isGeneratingData) {
      ...
      new Promise<void>(resolve => {
        ...
        // eslint-disable-next-line no-console
        console.warn(`Failed to generate data ${error}`);
      });
    }
  }
