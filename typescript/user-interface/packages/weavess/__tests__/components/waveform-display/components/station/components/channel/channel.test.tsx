...
const channelSegmentMap = new Map<string, WeavessTypes.ChannelSegment>();
channelSegmentMap.set(WeavessTypes.UNFILTERED, channelSegment);
...
const props: ChannelProps = {
  ...
  channel: {
    ...
    waveform: {
      ...
      channelSegments: channelSegmentMap,
      masks: [
        {
          id: 'mask',
          color: 'red',
  ...
  glMin: 0,
  glMax: 100,
  displayStartTimeSecs: timeRange.startTimeSecs,
  displayEndTimeSecs: timeRange.endTimeSecs,
  ...
  getViewRange: jest.fn().mockReturnValue([1, 4]),
  onMouseDown: jest.fn(),
  onMouseMove: jest.fn(),
  onMouseUp: jest.fn(),
...
describe('Weavess Channel', () => {
  test('channel wrapper to match snapshot', () => {
  ...
  it('call updateAmplitude', () => {
    expect(async () => channelInstance.updateAmplitude()).not.toThrow();
  });
...
describe('Weavess Channel private method', () => {
  ...
  describe('test keyboard events', () => {
    it('call onWaveformKeyDown amplitude scaling', () => {
      expect(() =>
        instance.onWaveformKeyDown(buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyS', true)))
      ).not.toThrow();

      // Set the amplitude scale keyDown followed by the keyup
      expect(() =>
        instance.onWaveformKeyDown(buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyS', false)))
      ).not.toThrow();
    ...
    it('call single channel reset amplitude scaling', () => {
      ...
      const resetKey = buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyS', true));
  ...
  it('call setWaveformYAxisBounds', () => {
    // Set it then change max but keep min the same
