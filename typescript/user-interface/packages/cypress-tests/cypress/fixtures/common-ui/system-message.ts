...
const pressSystemMessageScroll = () =>
  cy.get(selectors.systemMessageAutoScroll).first().click({ force: true });
...
const goToPreviousPage = () => {
  cy.get(selectors.tablePaging).contains('Previous').click();
};
...
export const pressPreviousPageButton = (): void => {
  goToPreviousPage();
  verifyNewMessageButtonExists();
};

export const pressNewMessagesButton = (): void => {
  cy.get(selectors.newMessagesButton).first().click();
  verifyNewMessageButtonDoesNotExists();
  cy.wait(Common.LONG_WAIT_TIME);
  verifyScrolledToBottom();
};

export const pressScrolling = (): void => {
 ...
 */
export const pressClearSystemMessages = (): void => {
...
export const openSystemMessageDisplay = (): void => {
