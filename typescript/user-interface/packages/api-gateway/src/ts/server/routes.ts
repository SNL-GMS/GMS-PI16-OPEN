...
import { HealthCheckResponse, healthChecks, HealthStatus } from './health-checks';
...
const routes: Routes = {
  ...
  healthChecks: {
    ...
    apollo: `/${appName}/health-check/apollo`,
    apolloInternal: `/${appName}/graphql`,
    websocket: `/${appName}/health-check/websocket`
  },
  authentication: {
    login: `/${appName}/auth/logInUser`,
    logout: `/${appName}/auth/logOutUser`,
    check: `/${appName}/auth/checkLogIn`
  }
};
 ...
 * @param protocol the http protocol
 */
export const configureRouteCheckApollo = (app: Express, protocol: string): void => {
  const handler = async (req, res) => {
    ...
    // check that graphql is there and that it responded with 400 since a query was not provided
    const condition = (c: HealthCheckResponse) =>
      // eslint-disable-next-line @typescript-eslint/no-magic-numbers
      c.code === 400 && c.value === 'GET query missing.';
    ...
    return res
      ...
      .send(condition(check) ? HealthStatus.OK : HealthStatus.FAILED);
  };
