...
import { EntityReference, Faceted } from '../../faceted';
// Dependency cycle is intentionally part of the architecture of Channel/Station data model
// eslint-disable-next-line import/no-cycle
import { Station } from '../station-definitions/station-definitions';
import * as ResponseTypes from './response-definitions';
...
export interface Channel extends Faceted {
   ...
   */
  readonly name: string;
  // TODO: this if from the API-Gateway will need to deal with it when station definitions are replaced in SOH app
  // displayName: string;
  /**
...
/**
 * Represents a Channel with just the name (Channel Facetted to minimum entries)
 * Used by UI to build a waveform query request
 */
export interface ChannelRequest extends Faceted {
  name: string;
  effectiveAt: number;
}
