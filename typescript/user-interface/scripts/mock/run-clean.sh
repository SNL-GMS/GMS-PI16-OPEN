...
function explain_basic_usage {
  ...
  echo "Brings down the mocker docker containers. Then stops each container. Then prunes containers, volumes, and images. Then removes the frameworks-data-injector image. Then kills the mock data server. Then deletes the mock data server docker image and the waveform manager service docker image."
...
function delete_docker_images {
  docker images | egrep "gms-common/mock-waveform-application|gms-common/mock-data-server" | egrep -i ${VERSION:-develop} | awk '{print $3}' | xargs -I % docker rmi -f %
}
...
function clean_up {
  ...
  if [[ $GMS_UI_MODE =~ ^(soh|SOH|Soh)$ ]] ; then
    ...
    image_hash="$(docker image ls -aqf 'reference=$CI_DOCKER_REGISTRY/gms-common/frameworks-data-injector:latest')"
