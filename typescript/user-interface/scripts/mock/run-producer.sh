...
function explain_basic_usage {
  ...
  echo "By default, it produces station-soh, capability-rollup, acknowledging, quieting, and system-messages"
  echo ""
...
function init_producer {
  ...
  topic=soh.ui-materialized-view
...
function start_system_message_producer {
  start_producer SYSTEM_MESSAGE systemMessage.json system.system-messages
}
...
function validate_is_built {
  validate_image_exists "$CI_DOCKER_REGISTRY/gms-common/frameworks-data-injector"
...
if [ "$soh_producer" = true ] ; then
  start_producer $injectable_type "baseInjectableUiStationAndStationGroups.json" $topic $mount
fi
